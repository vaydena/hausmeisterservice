...
/**
 * Sprint 42 · Portal-Datenauskunft-Download.
 * Analog zum Staff-ExportDataButton, ruft aber /api/portal/privacy/export
 * an — die Route selbst gated per requireResidentContext und sammelt nur
 * bewohnerrelevante Tabellen.
 */
export function PortalExportButton() {
  ...
  async function handleDownload() {
    setPending(true);
    try {
      const res = await fetch('/api/portal/privacy/export', {
        method: 'GET',
      ...
      const filename = match?.[1] ?? 'portal-datenauskunft.json';
